...
mod tests {
    ...
    async fn test_cloudflare() {
        ...
        driver
            .bypass_cloudflare("https://nowsecure.nl")
            .await
            .expect("Failed to bypass Cloudflare");
        tokio::time::sleep(std::time::Duration::from_secs(5)).await;
        println!(
            "{}",
            driver.source().await.expect("Failed to get driver source")
        );
        ...
        assert_eq!(
            passed
                .first()
                .await
                .expect("Failed to find passed element")
                .text()
                .await
                .expect("Failed to get text of passed element"),
            "you passed!"
        );
